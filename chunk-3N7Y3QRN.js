...
HAVING COUNT(*) >= 5
ORDER BY user_id, streak_start;`},{type:"callout",variant:"info",title:"Gaps and Islands",body:"For consecutive dates, `date - row_number \xD7 one day` stays constant. That constant identifies an island. Deduplicating by user/day first prevents several logins on one day from inflating the streak."}]},{id:"sargability",title:"5. Functions in WHERE and SARGability",blocks:[{type:"callout",variant:"tip",title:"Related reading",body:"Index types, B-trees, and when to create them: [Database Indexes](/designs/database-indexes)."},{type:"code",language:"sql",filename:"sargable_predicates.sql",showLineNumbers:!0,code:`-- Non-SARGable for a normal index on balance:
...
The first diagnosis is usually **plan + data shape**. Do not add an index based only on development timing; fix bad estimates/query shape/indexing and verify total load and p95/p99 after the change.`}]}]}]},a=t;export{a as default};
