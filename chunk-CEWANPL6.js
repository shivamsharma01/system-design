import{a as e}from"./chunk-YUBYTNTW.js";import"./chunk-IFGU66OU.js";var a={meta:e,sections:[{id:"overview",title:"Overview",blocks:[{type:"markdown",value:"**Load balancing** spreads incoming traffic across a pool of healthy backends so no single instance becomes a bottleneck. It is the foundation of horizontal scale: add capacity by adding instances, and let the balancer decide *where* each request goes. Broader context: [Scalability](/designs/scalability), [Avoiding SPOFs](/designs/single-point-of-failure), [Proxy vs Reverse Proxy](/designs/proxy-vs-reverse-proxy)."},{type:"callout",variant:"info",title:"What interviewers want",body:"Name **L4 vs L7**, pick an **algorithm** with a reason, explain **health checks** and **draining**, and know when **sticky sessions** hurt. Contrast load balancers with **API Gateway** (edge features) and **service mesh** (east-west policy)."},{type:"table",caption:"Where load balancing shows up.",headers:["Layer","Example"],rows:[["DNS / GSLB","Route users to the nearest region"],["Edge / reverse proxy","Nginx, HAProxy, AWS ALB in front of app pods"],["L4 network","AWS NLB, LVS \u2014 TCP/UDP without inspecting HTTP"],["Client-side","gRPC client pickers, Envoy sidecar, Consuls"],["Database / cache","ProxySQL, Redis Cluster hashing"]]}]},{id:"l4-vs-l7",title:"L4 vs L7",blocks:[{type:"markdown",value:"**Layer 4 (transport)** balances on IP/port and TCP/UDP connection state. It is fast, protocol-agnostic, and cannot route on HTTP path or headers. **Layer 7 (application)** terminates (or inspects) HTTP/gRPC and can route by host, path, cookie, or header \u2014 at higher CPU and latency cost."},{type:"table",caption:"L4 vs L7 at a glance.",headers:["Dimension","L4 (NLB, LVS)","L7 (ALB, Nginx http, Envoy)"],rows:[["Sees","IP, port, TCP/UDP","HTTP method, path, headers, cookies"],["Routing","Per connection / 5-tuple","Per request; content-based"],["TLS","Often passthrough (or terminate once)","Usually terminate; can re-encrypt"],["Latency / throughput","Very high throughput, low overhead","More CPU; richer features"],["Use when","Raw TCP, gaming, Kafka, extreme QPS","Microservices HTTP, path routing, WAF"]]},{type:"callout",variant:"tip",title:"AWS shorthand",body:"**NLB** \u2248 L4 (connection-level, static IPs, ultra low latency). **ALB** \u2248 L7 (HTTP/HTTPS, path/host rules, target groups). Many stacks use NLB \u2192 Envoy/ALB for both speed and smart routing."},{type:"mermaid",caption:"Edge L7 balancer in front of a service pool.",definition:`flowchart LR
  ...
  HC[Health checks] -.-> A
  HC -.-> B
  HC -.-> C`}]},{id:"algorithms",title:"Balancing Algorithms",blocks:[{type:"markdown",value:"Pick the algorithm from **traffic shape** and **session affinity** needs \u2014 not from habit. Interview answers should justify the choice. Also see [Consistent Hashing](/designs/consistent-hashing) for ring-based affinity."},{type:"image",src:"assets/article-images/load-balancing-pattern/01-overview.png",alt:"Load balancer distributing requests across multiple backend servers",caption:"Balancer sits in front of a server pool. Diagram adapted from Ashish Pratap Singh / AlgoMaster."},{type:"markdown",value:`### 1. Round Robin

Cycle through servers in order. Simple and even for **homogeneous** backends with similar request cost.`},{type:"image",src:"assets/article-images/load-balancing-pattern/02-round-robin.png",alt:"Round robin assigning requests to servers in cyclic order",caption:"Request 1\u2192S1, 2\u2192S2, 3\u2192S3, 4\u2192S1\u2026. Diagram adapted from Ashish Pratap Singh / AlgoMaster."},{type:"code",language:"java",filename:"RoundRobin.java",code:`public final class RoundRobin {
...
}`},{type:"prosCons",title:"Round Robin",pros:["Simple","Even distribution on identical servers"],cons:["Ignores live load","Slow server still gets equal share"]},{type:"markdown",value:`### 2. Weighted Round Robin

Assign **weights** by capacity (CPU/RAM). Higher weight \u2192 proportionally more requests. Good for mixed VM sizes or canaries.`},{type:"image",src:"assets/article-images/load-balancing-pattern/03-weighted-round-robin.png",alt:"Weighted round robin giving more traffic to higher-capacity servers",caption:"Weights steer share of traffic. Diagram adapted from Ashish Pratap Singh / AlgoMaster."},{type:"image",src:"assets/article-images/load-balancing-pattern/04-weighted-rr-code.png",alt:"Code sketch of weighted round robin implementation",caption:"Reference sketch. Diagram adapted from Ashish Pratap Singh / AlgoMaster."},{type:"code",language:"java",filename:"WeightedRoundRobin.java",code:`public final class WeightedRoundRobin {
...
}`},{type:"markdown",value:`### 3. Least Connections

Send the next request to the server with the **fewest active connections**. Ideal when request durations vary.`},{type:"image",src:"assets/article-images/load-balancing-pattern/05-least-connections.png",alt:"Least connections routing to the server with the fewest open connections",caption:"Pick the least-busy server by connection count. Diagram adapted from Ashish Pratap Singh / AlgoMaster."},{type:"image",src:"assets/article-images/load-balancing-pattern/06-least-connections-code.png",alt:"Code sketch of least connections load balancer",caption:"Reference sketch. Diagram adapted from Ashish Pratap Singh / AlgoMaster."},{type:"code",language:"java",filename:"LeastConnections.java",code:`record Backend(String id, AtomicInteger active) {}
...
}`},{type:"markdown",value:`### 4. Least Response Time

Prefer the server with the **lowest recent latency** (often combined with connection count). Best when response times differ across the pool.`},{type:"image",src:"assets/article-images/load-balancing-pattern/07-least-response-time.png",alt:"Least response time routing to the fastest responding server",caption:"Optimize for measured latency. Diagram adapted from Ashish Pratap Singh / AlgoMaster."},{type:"image",src:"assets/article-images/load-balancing-pattern/08-least-response-time-code.png",alt:"Code sketch of least response time load balancer",caption:"Reference sketch. Diagram adapted from Ashish Pratap Singh / AlgoMaster."},{type:"code",language:"java",filename:"LeastResponseTime.java",code:`record TimedBackend(String id, AtomicLong ewmaMs, AtomicInteger active) {}
...
}`},{type:"markdown",value:"### 5. IP Hash\n\n`hash(clientIp) % N` pins a client to a backend \u2014 simple **session persistence** without cookies. Watch NAT (many users, one IP) and remapping when N changes \u2014 prefer [consistent hashing](/designs/consistent-hashing) for caches."},{type:"image",src:"assets/article-images/load-balancing-pattern/09-ip-hash.png",alt:"IP hash mapping client IPs to fixed backend servers",caption:"Same client IP \u2192 same server. Diagram adapted from Ashish Pratap Singh / AlgoMaster."},{type:"image",src:"assets/article-images/load-balancing-pattern/10-ip-hash-code.png",alt:"Code sketch of IP hash load balancer",caption:"Reference sketch. Diagram adapted from Ashish Pratap Singh / AlgoMaster."},{type:"code",language:"java",filename:"IpHash.java",code:`public final class IpHash {
  private final List<String> servers;
...
}`},{type:"image",src:"assets/article-images/load-balancing-pattern/11-algorithm-summary.png",alt:"Summary comparison of five load balancing algorithms",caption:"When to use each algorithm. Diagram adapted from Ashish Pratap Singh / AlgoMaster."},{type:"table",caption:"Quick pick guide.",headers:["Algorithm","Best for","Watch out"],rows:[["Round Robin","Homogeneous servers","Ignores load"],["Weighted RR","Mixed capacity / canary","Weights must match reality"],["Least Connections","Uneven request duration","Needs accurate counts"],["Least Response Time","Latency-sensitive pools","Needs good metrics"],["IP Hash","Simple stickiness","NAT skew; remaps on N change"]]},{type:"callout",variant:"warning",title:"Sticky sessions trade-offs",body:"Cookie or IP stickiness simplifies in-memory sessions but **hurts scale-out and draining**. Prefer **external session store** (Redis) and **stateless apps**. Use stickiness only when you must (WebSocket affinity, legacy apps)."},{type:"callout",variant:"note",title:"Source",body:"Algorithm diagrams and walkthroughs summarized from Ashish Pratap Singh\u2019s AlgoMaster article \u201CLoad Balancing Algorithms Explained with Code (and Visuals),\u201D with Java sketches for this platform."}]},{id:"health-and-draining",title:"Health Checks and Connection Draining",blocks:[{type:"markdown",value:"A balancer that ignores health is a **random outage distributor**. Health checks remove bad targets; **connection draining** (deregistration delay) finishes in-flight work before the instance dies."},{type:"table",headers:["Mechanism","Purpose"],rows:[["Liveness probe","Process alive? Restart if not (orchestrator)"],["Readiness / target health","Ready for traffic? Balancer stops sending if not"],["Deep health","Can reach DB / dependency? Use carefully \u2014 cascading fails"],["Connection draining","Stop new requests; wait for active ones (e.g. 30\u2013300s)"],["Outlier detection","Eject hosts with high error rate (Envoy) without full redeploy"]]},{type:"mermaid",caption:"Deploy with drain before terminate.",definition:`sequenceDiagram
...
4. Software (Nginx, Envoy, ALB/NLB) dominates; compose with **API Gateway** and **mesh**, do not confuse them.`}]}]},s=a;export{s as default};
